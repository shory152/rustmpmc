...
pub fn run_mpsc_test<T:'static+NewData+Send+Sync>(nSender : i32, nReceiver : i32, durationS : i32)
    -> Result<(), &'static str> {
    if nReceiver != 1 {
        return Err("MPSC channel does not support multiple consumers");
    }
    println!("========= test std mpsc channel ==========");
    ...
    //dataQ.close();
    unsafe{
        let pStop = Arc::into_raw(stopped);
        *(pStop as *mut bool) = true;
        stopped = Arc::from_raw(pStop);
    }
